...
export function AtencionesEnfermeriaCreatePage({
  ...
  if (lookupQuery.isError || lookupQuery.data === undefined) {
    return (
      <section
        ...
        aria-labelledby="atenciones-enfermeria-create-error-title"
      >
        <p className="eyebrow">Enfermería</p>
  ...
  return (
    <section
      ...
      aria-labelledby="atenciones-enfermeria-create-title"
    >
      ...
      <AtencionesEnfermeriaForm
        ...
        error={
          createMutation.error === null
            ? null
            : resolveAtencionesEnfermeriaApiError(createMutation.error)
        }
        onCancel={() => navigate(ATENCIONES_ENFERMERIA_PATH)}
        onOpenMedicalAttention={(atencionId) =>
          navigate(buildAtencionIndividualDetailPath(adultoMayorId, atencionId))
        }

...
describe("actividades grupales filter state", () => {
  ...
  it("upgrades saved filters for the organizer roles that now share a series", () => {
    window.sessionStorage.setItem(
      "cuidarte:actividades-grupales:filters:user-3",
      JSON.stringify({ organizer: "enfermeria" }),
    );

    expect(loadActividadesGrupalesFilters("user-3", fallbackFilters)).toEqual({
      ...fallbackFilters,
      organizer: "medico",
    });
  });
